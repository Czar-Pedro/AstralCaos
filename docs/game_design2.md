...
Última atualização: 14/08/2026 (Velocidade derivada de Destreza; decisão de não ter atributo Sorte dedicado — crítico limitado por teto de chance e multiplicador moderado para evitar builds quebradas)
...
| Sistema | Status |
|---|---|
| Personagens (jogador + NPCs) | Direções gerais definidas |
| Inimigos | Direções gerais definidas |
| Combate | Direções gerais definidas |
| Equipamento | Direções gerais definidas (runas aguardam design do Runista) |
...
- Direção definida: pontos livres a cada level up (ex: 5, valor exato a definir), mais um pequeno bônus passivo no atributo principal/secundário da classe.
- Objetivo: manter liberdade de build sem a classe perder identidade (evita, por ex., um Mago virar tanque puro só de distribuição). Não é impossível de outra forma — só dá mais trabalho pra montar/balancear sem esse bônus de classe.
...
- Ainda falta: valor exato de pontos por nível, e o **teto de nível** (level cap) do jogo.
...
- Direção definida: pontos de afinidade por NPC, de 0 a 100, em faixas: **Estranho / Conhecido / Confiante / Aliado / Vínculo**.
- Ganha pontos por: presença em batalha junto (passivo por batalha), escolhas de diálogo específicas, presentes (itens com like/dislike por NPC), e conclusão de quests pessoais do NPC (salto grande de afinidade).
- **Carisma do jogador impacta o ganho de afinidade — para o bem e para o mal**: Carisma alto acelera ganho de afinidade em interações positivas, mas também pode intensificar a perda em interações negativas (ex: uma escolha de diálogo ruim custa mais afinidade num personagem carismático — a lógica sendo que suas palavras têm mais peso, positivo ou negativo).
...
- Ainda falta: valores exatos das faixas (pontos por ação, limites de cada uma), a fórmula exata de como Carisma modifica o ganho/perda de afinidade, e o que cada faixa de Vínculo desbloqueia especificamente por NPC.
...
- Direção definida: cada raça é caracterizada principalmente por **passivas de sabor** (até ~5 por raça), não por peso numérico em atributos — isso mantém as 27 raças viáveis sem explodir o trabalho de balanceamento.
...
### Em aberto
- Detalhar melhor os pontos ainda soltos dentro de cada subseção acima (passivas raciais completas, valores exatos de afinidade)
...
**Crítico:**
- Chance de crítico vem de uma parcela pequena de Destreza como base, mas a maior parte do investimento em crítico vem de **skills e runas** (não existe atributo Sorte dedicado — ver seção 1, decisão tomada para evitar builds de crítico que empilham atributo + skill + runa e ficam quebradas).
...
| Raridade | Cor sugerida | O que muda |
|---|---|---|
| Comum | Branco/Cinza | Stats base, sem slot de runa (ou 1 slot) |
| Incomum | Verde | Stats um pouco melhores, 1-2 slots |
| Raro | Azul | Stats bons, 2-3 slots, pode ter 1 efeito fixo |
| Épico | Roxo | Stats altos, 3 slots, 1-2 efeitos fixos |
| Lendário | Dourado/Laranja | Stats máximos, 3-4 slots, efeito único exclusivo |
